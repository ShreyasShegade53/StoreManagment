...
import { EntityArrayResponseType, SoldProductService } from '../service/sold-product.service';
...
export class SoldProductComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISoldProduct): number => this.soldProductService.getSoldProductIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.soldProducts = this.refineData(dataFromBody);
  }

  protected refineData(data: ISoldProduct[]): ISoldProduct[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ISoldProduct[] | null): ISoldProduct[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.soldProductService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
